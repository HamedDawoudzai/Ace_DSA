...
import { AttemptRequest } from "../types";
...
export default function DrillDetailScreen() {
  ...
  }, [step, celebrationScale, celebrationRotate]);
  ...
  const handleApproachSelect = (index: number) => {
    ...
    if (isCorrect) {
      ...
      setApproachDisabled((prev) => new Set(prev).add(index));
      animateHint();
    }
  };
  ...
  const currentSlideStyle =
    step === "approach"
      ? {
          opacity: entranceAnim.interpolate({ inputRange: [0, 1], outputRange: [0, 1] }),
          transform: [
            {
  ...
  return (
    ...
    >
      <Animated.View
        style={[
          currentSlideStyle,
          step !== "approach" && step !== "complete" ? {} : {},
        ]}
      >
        {step === "approach" && (
          <View>
            <View style={styles.headerRow}>
              <View style={[styles.categoryPill, { backgroundColor: colors.accentSubtle }]}>
            ...
            {approachCorrect && (
              ...
              >
                ...
                <Text style={[styles.correctBannerText, { color: colors.success }]}>
                  Correct! Moving on...
                </Text>
              </View>
            )}
            ...
            {showExplainButton && step === "approach" && drill.explanation !== "" && (
              <ExplainButton colors={colors} isDark={isDark} onPress={() => setShowExplainModal(true)} />
            )}
          </View>
        )}
          ...
          <View>
            <View
              style={[
                ...
                {
                  backgroundColor: isDark ? "rgba(13,217,196,0.06)" : "rgba(240,165,0,0.06)",
                  borderColor: isDark ? "rgba(13,217,196,0.2)" : "rgba(240,165,0,0.2)",
                },
              ]}
            >
            ...
            {showExplainButton && step === "complexity" && drill.explanation !== "" && (
              <ExplainButton colors={colors} isDark={isDark} onPress={() => setShowExplainModal(true)} />
            )}
          </View>
        )}
            ...
            >
              ...
              <View style={styles.summaryRow}>
                <View style={styles.summaryItem}>
                  ...
                  <Text style={[styles.summaryLabel, { color: colors.textMuted }]}>Approach</Text>
                  <Text style={[styles.summaryValue, { color: colors.text }]} numberOfLines={2}>
                    {drill.choices[drill.correct_option]}
                  </Text>
                </View>
                <View style={[styles.summaryDividerV, { backgroundColor: colors.border }]} />
                <View style={styles.summaryItem}>
                  <Ionicons name="speedometer-outline" size={18} color={colors.accent} />
                  <Text style={[styles.summaryLabel, { color: colors.textMuted }]}>Complexity</Text>
                  <Text style={[styles.summaryValue, { color: colors.text }]} numberOfLines={2}>
              ...
              <Pressable
                ...
                onPress={() => navigation.navigate("Drills", { advance: true })}
              >
  ...
  return (
    <Animated.View style={{ transform: [{ scale: scaleAnim }], marginBottom: 10 }}>
      ...
      >
        ...
        <Text style={[styles.choiceText, { color: isDisabled ? colors.textMuted : colors.text }]}>
          {text}
        </Text>
        {isCorrectChoice && (
          <Ionicons name="checkmark-circle" size={22} color={colors.success} style={styles.choiceIcon} />
        )}
        {isWrongChoice && (
          <Ionicons name="close-circle" size={22} color={colors.error} style={[styles.choiceIcon, { opacity: 0.6 }]} />
        )}
      </Pressable>
    </Animated.View>
  );
...
function ExplainButton({
  ...
  const scaleAnim = useRef(new Animated.Value(1)).current;
  return (
    <Animated.View style={{ transform: [{ scale: scaleAnim }], marginTop: 14, alignSelf: "flex-end" }}>
      <Pressable
        style={[
          explainStyles.btn,
          {
      ...
      >
        <Ionicons name="bulb" size={16} color={isDark ? "#FBBF24" : "#D97706"} />
        <Text style={[explainStyles.btnText, { color: isDark ? "#FBBF24" : "#D97706" }]}>
          Explain
        </Text>
      </Pressable>
    </Animated.View>
    ...
    <Modal visible={visible} animationType="slide" transparent={false}>
      <View style={[explainStyles.modal, { backgroundColor: colors.background }]}>
        <View style={explainStyles.modalHeader}>
          <Text style={[explainStyles.modalTitle, { color: colors.text }]} numberOfLines={2}>
            {prompt}
          </Text>
          <Pressable
          ...
          return (
            ...
            >
              <View style={explainStyles.codeLangTag}>
                <Text style={[explainStyles.codeLangText, { color: colors.textMuted }]}>Python</Text>
              </View>
              <Text
                style={[
...
function FormattedText({ text, colors }: { text: string; colors: any }) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (boldMatch) {
      elements.push(
        <Text key={`b-${i}`} style={[explainStyles.boldLine, { color: colors.text }]}>
    ...
    if (stepMatch) {
      elements.push(
        <View key={`s-${i}`} style={explainStyles.stepRow}>
          <Text style={[explainStyles.stepNumber, { color: colors.accent }]}>{stepMatch[1]}.</Text>
          <Text style={[explainStyles.stepText, { color: colors.text }]}>{stepMatch[2]}</Text>
        </View>
      );
      continue;
    }
    ...
    elements.push(
      <Text key={`t-${i}`} style={[explainStyles.normalText, { color: colors.text }]}>
        {line}
      </Text>
    );
  }
  ...
  return (
    <View
      style={[
        styles.hintCard,
        {
          backgroundColor: isDark ? "rgba(251,191,36,0.08)" : "rgba(251,191,36,0.10)",
          borderColor: isDark ? "rgba(251,191,36,0.25)" : "rgba(251,191,36,0.35)",
        },
      ]}
    >
      <Ionicons name="bulb" size={20} color={isDark ? "#FBBF24" : "#D97706"} style={styles.hintIcon} />
      <Text style={[styles.hintText, { color: isDark ? "#FDE68A" : "#92400E" }]}>{text}</Text>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  categoryPill: { borderRadius: 999, paddingHorizontal: 12, paddingVertical: 5 },
  ...
  letterCircle: { width: 34, height: 34, borderRadius: 17, alignItems: "center", justifyContent: "center" },
  ...
  referenceHeader: { flexDirection: "row", alignItems: "center", gap: 6, marginBottom: 8 },
  referenceLabel: { fontSize: 12, fontWeight: "600", textTransform: "uppercase", letterSpacing: 0.5 },
  referenceText: { fontSize: 15, fontWeight: "600", lineHeight: 21 },
  complexityHeader: { fontSize: 18, fontWeight: "700", lineHeight: 25, marginBottom: 18 },
  celebrationWrapper: { paddingTop: 20 },
  celebrationCard: { borderRadius: 20, borderWidth: 1.5, padding: 28, alignItems: "center" },
  ...
  celebrationTitle: { fontSize: 24, fontWeight: "800", letterSpacing: -0.3, marginBottom: 6 },
  ...
  summaryValue: { fontSize: 13, fontWeight: "600", textAlign: "center", lineHeight: 18 },
...
const explainStyles = StyleSheet.create({
  ...
  codeLangText: { fontSize: 11, fontWeight: "600", textTransform: "uppercase", letterSpacing: 0.5 },
  codeText: {
  ...
  stepRow: { flexDirection: "row", paddingLeft: 4, marginVertical: 3, paddingRight: 16 },
